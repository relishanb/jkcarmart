  ...
  return (
    <div
        className={`fixed inset-0 bg-black bg-opacity-50 z-20 transition-opacity duration-300 ${
          isSidebarActive ? "visible opacity-100" : "invisible opacity-0"
        }`}
        onClick={toggleSidebar} 
      >
        <div
          className={`bg-white w-72 h-full absolute top-0 left-0 z-50 transform transition-transform duration-300 ease-in-out ${
          ...
          <div className="p-4 py-6">
            {authentication.isLoggedIn ? (
              <div className="border-b pb-4">
                ...
                <p className="text-sm font-medium text-black">{authentication.mobileNumber}</p>
                <button className="text-orange-500 text-sm ">Edit profile &gt;</button>
              </div>
            ) : (
            ...
            <div className="mt-4">
              ...
              <Button className="flex justify-center gap-2 items-center w-full text-center bg-orange-500 text-white font-medium py-2 rounded-md text-lg">
              {isAndroid && <IoLogoGooglePlaystore size={24}/>}
              {isIOS && <IoLogoApple size={24}/> }
                Get the App
              </Button>
              </Link>
            </div>
